...
const standaloneClientPath = path.join(__dirname, '..', '.next', 'standalone', 'node_modules', '.prisma', 'client');

// Also copy to @prisma/client for imports
const prismaCoreClientPath = path.join(__dirname, '..', 'node_modules', '@prisma', 'client');
const standaloneCoreClientPath = path.join(__dirname, '..', '.next', 'standalone', 'node_modules', '@prisma', 'client');
...
try {
  ...
  if (!fs.existsSync(standalonePath)) {
    console.log('⚠️  .next/standalone not found. This script should run after "next build"');
  ...
  if (fs.existsSync(prismaClientPath)) {
    ...
    console.log('   ⚠️  .prisma/client not found. Run "prisma generate" first.\n');
  }
  ...
  const engineFiles = fs.readdirSync(standaloneClientPath).filter(file => 
